...
func getConcatenateBaseString() string {
	...
	if requestBody != "" {
		...
		requestDataBytes := []byte(requestBody)
		json.Unmarshal(requestDataBytes, &requestDataMap)

		requestDataJsonByte, err := JSONMarshal(requestDataMap)
		if err != nil {
			fmt.Println("Error occurred: " + err.Error())
		}

		// Removing last line generated by JSONMarshal()
		requestDataJson := strings.TrimSuffix(string(requestDataJsonByte), "\n")

		requestDataJson = url.QueryEscape(requestDataJson)

		//Replacing each occurrence of "+" to "%20"
		requestDataJson = strings.Replace(requestDataJson, "+", "%20", -1)
		baseArray = baseArray + "&" + requestDataJson
	}
	return baseArray
}
